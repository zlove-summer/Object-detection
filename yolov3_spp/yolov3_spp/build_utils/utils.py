...
def compute_loss(p, targets, model):  # predictions, targets, model
    device = p[0].device #去除device
    # 建立分类损失、定位损失、置信度损失空数组
    ...
    # Define criteria，reduction是loss求平均操作
    ...
    for i, pi in enumerate(p):  # layer index, layer predictions
        ...
        if nb:
            ...
            # GIoU
            pxy = ps[:, :2].sigmoid() #预测值经过sigmoid函数
            ...
            # Class
            if model.nc > 1:  # cls loss (only if multiple classes)，类别数大于时才计算
    ...
    # loss = lbox + lobj + lcls，以字典形式，返回最后的损失
    return {"box_loss": lbox,
            "obj_loss": lobj,
            "class_loss": lcls}


def build_targets(p, targets, model): #targets是目标边界框
    # Build targets for compute_loss(), input targets(image_idx,class,x,y,w,h)
    nt = targets.shape[0]  #取出目标数量
    ...
    for i, j in enumerate(model.yolo_layers):  # j: [89, 101, 113]
        ...
        # 注意anchor_vec是anchors缩放到对应特征层上的尺度（原anchor缩放对应倍数）
        ...
        if nt:  # 如果存在target的话
            ...
            a, t = at[j], t.repeat(na, 1, 1)[j]  # filter，t是所有正样本
