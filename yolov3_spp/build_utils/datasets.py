...
def exif_size(img):
    ...
    try: #如果图片是有旋转的，长宽的值会进行变换（如旋转90度）
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self,
                 ...
                 img_size=416, #经过预处理后图像尺度
                 batch_size=16,
                 augment=False,  # 训练集设置为True(augment_hsv)，颜色亮度等增强，验证集设置为False
                 hyp=None,  # 超参数字典，其中包含图像增强会使用到的超参数，hyp.yaml文件，最后会解析为字典形式
                 rect=False,  # 是否使用rectangular training
                 cache_images=False,  # 是否缓存图片到内存中
                 single_cls=False, pad=0.0, rank=-1): #通过rank判断哪个进程

        try:
            path = str(Path(path)) #标准化path路径
            # parent = str(Path(path).parent) + os.sep
            # path路径所指是否是一个文件
            if os.path.isfile(path):  # file
                # 读取对应my_train/val_data.txt文件，读取每一行的图片路劲信息
                with open(path, "r") as f:
                    f = f.read().splitlines() #按行分割
            else:
                raise Exception("%s does not exist" % path)

            # 检查每张图片后缀格式是否在支持的列表中，保存支持的图像路径
            # img_formats = ['.bmp', '.jpg', '.jpeg', '.png', '.tif', '.dng']
            # os.path.splitext分离文件名和扩展名，-1取的是扩展名
        ...
        # batch index
        # 将数据划分到一个个batch中，生成的bi可以理解为mask
        ...
        # 如果为ture，训练网络时，会使用类似原图像比例的矩形(让最长边为img_size，短边等比例缩放)，而不是img_size x img_size
        # 注意: 开启rect后，mosaic就默认关闭
        if self.rect:
        ...
        if os.path.isfile(np_labels_path): #加载标签
        ...
        for i, file in enumerate(pbar):
            ...
            if l.shape[0]:
                ...
                # extract_bounding_boxes可以将每一个目标裁剪出来，最后作为分类
        ...
        if cache_images:  # if training，将内盘上的图片缓存到进程当中
        ...
        detect_corrupted_images = False #检测图片是否有破损
    ...
    def __getitem__(self, index): #通过index索引，拿到图片、标签等信息
        hyp = self.hyp #超参数
        if self.mosaic: #是否使用mosaic增强，训练时默认使用mosaic方法
            # load mosaic
    ...
    @staticmethod
    def collate_fn(batch): #如何将数据集进行打包的函数
...
def load_image(self, index):
    ...
    img = self.imgs[index]
    if img is None:  # not cached，表示缓存当中没有图片
        path = self.img_files[index]
        img = cv2.imread(path)  # BGR
        assert img is not None, "Image Not Found " + path
        h0, w0 = img.shape[:2]  # orig hw
        # img_size 设置的是预处理后输出的图片尺寸，将图片的最大变成resize到指定边长，图片比例不变
        r = self.img_size / max(h0, w0)  # resize image to img_size
        if r != 1:  # if sizes are not equal
            interp = cv2.INTER_AREA if r < 1 and not self.augment else cv2.INTER_LINEAR
            img = cv2.resize(img, (int(w0 * r), int(h0 * r)), interpolation=interp)
        # 返回图片，原始的图片大小，resize后的图片大小
        return img, (h0, w0), img.shape[:2]  # img, hw_original, hw_resized
    else:
        return self.imgs[index], self.img_hw0[index], self.img_hw[index]  # img, hw_original, hw_resized


def load_mosaic(self, index): #mosaic数据增强方法
    """
    将四张图片拼接在一张马赛克图像中
    :param self:
    ...
    s = self.img_size #期待拼接好后输出的图像尺寸
    # 随机初始化拼接图像的中心点坐标，中心点坐标在4倍图像的中间部分（0.5，1.5）
    ...
    for i, index in enumerate(indices):
        ...
        # 计算padw和padh，是为了计算将标注框的坐标改变成拼接后的图像的坐标
        padw = x1a - x1b
        padh = y1a - y1b
